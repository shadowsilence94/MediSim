...
\begin{abstract}
Traditional digital healthcare systems struggle to process unstructured multimodal patient inputs and complex triage queries safely. We propose MediSim, a unified system featuring a Multimodal Diagnostic Assistant and a Multi-Agent Triage framework. By fusing CNN and biLSTM architectures for images and text, and orchestrating role-based LLM agents for triage, we establish a self-verifying clinical pipeline. Preliminary designs suggest this hybrid architecture can reduce clinical hallucination errors by explicitly grounding generative outputs.

\paragraph{Keywords:} Multimodal AI, Agentic Triage, Medical Vision-Language, Clinical Safety.
\paragraph{ACM Classification:} Applied computing $\rightarrow$ Health informatics, Computing methodologies $\rightarrow$ Multi-agent systems.
\end{abstract}
...
% Paragraph 2: The Gap
However, existing diagnostic systems generally treat medical imaging and conversational triage as entirely disparate pipelines, requiring massive computational resources and exhibiting high risk for clinical hallucination when interacting with patients. While lightweight unimodal networks like CNNs and LSTMs perform symptom classification effectively \citep{mullenbach2018explainable}, they lack dynamic interaction. Conversely, end-to-end medical LLMs remain computationally dense and prone to unsafe generative behaviors in critical medical decision-making scenarios, requiring robust mitigation strategies \citep{sahu2025occutriage}. Specifically, the gap lies in the absence of a unified, lightweight system that can orchestrate standard CNN/LSTM pipelines for deterministic multimodal analysis while simultaneously operating a constrained, multi-agent LLM framework to cross-verify dynamic triage symptoms.

% Paragraph 3: Your Work
Therefore, we propose MediSim: a Multimodal Diagnostic and Agentic Triage System. MediSim comprises two selectable features. First, it processes synchronized image (Chest X-ray) and patient symptom text using an ensemble of CNN and biLSTM architectures to output preliminary diagnostic reports. Second, it facilitates interactive triage through a structured Multi-Agent framework comprising specific roles, including a Triage Nurse, Specialist Doctor, and Medical Fact-Checker, to engage the patient, gather dynamic symptoms, and propose verified clinical next steps. This methodology is promising because it segregates deterministic image-text classification from generative conversational AI, effectively grounding the generative agents with structured input while cross-verifying outputs via the Fact-Checker agent.

% Paragraph 4: Key Findings (Anticipated for Proposal)
While our system is currently in the initial design phase, we establish three core research questions using a structured experimental framework. We define our \textbf{Independent Variables (IV)} as the architectural modality (Unimodal vs. Multimodal Fusion) and the agent configuration (Single-Agent vs. Multi-Agent). Our \textbf{Dependent Variables (DV)} are diagnostic classification accuracy and clinical hallucination rates. Based on ablation studies of similar architectures \citep{lu2024triageagent}, we expect our multi-agent architecture to lower discordance rates with clinical guidelines by at least 15\% compared to standalone LLMs.
...
\paragraph{Lightweight Clinical Text Classification:}
Before the dominance of heavy transformer models, attention-based CNNs and Bidirectional LSTMs successfully dominated clinical concept extraction and patient risk assessment from free text. Studies show that CNN and LSTM character-level embeddings achieve highly competitive precision metrics on real-world clinical notes while drastically reducing training and inference latency \citep{mullenbach2018explainable, girardi2018patient}. We draw upon these established methods to build a computationally accessible baseline for interpreting patient symptom inputs.
...
\node[box, draw=green!60!black, thick, right=0.5cm of xray] (symptoms) {Symptoms\\ Text Input (BiLSTM)};
...
\node[box, draw=blue!60, below=0.4cm of cnn] (bilstm) {BiLSTM: Text Encoder};
...
The primary objective of this project is to develop and evaluate MediSim's dual-pipeline architecture. We hypothesize that (1) a fused CNN-biLSTM model will achieve higher diagnostic classification precision than isolated unimodal counterparts, and (2) our three-role multi-agent framework will produce a significantly lower hallucination rate relative to a single-agent baseline executing the same triage task.
...
\begin{enumerate}[label=\textbf{\arabic*.}]
    \item \textbf{Vision-Language Fusion}: An ensemble approach where image features are extracted using an established CNN (e.g., ResNet or DenseNet), while symptom text is embedded through a biLSTM network. The dense feature layers of both networks will be concatenated and fed through a classification head.
    \item \textbf{Multi-Agent Triage}: Utilizing accessible LLM APIs (e.g., Groq/Ollama), this component initializes three distinct agents using sophisticated system prompts: the Triage Nurse, the Specialist, and the Fact-Checker. 
\end{enumerate}

\subsection{Apparatus and Compute Environment}
Experiments will be conducted on consumer-grade hardware equipped with an Apple M-series silicon or NVIDIA RTX-series GPU (min. 8GB VRAM) to ensure real-world viability. LLM interaction will be managed via high-throughput inference APIs to maintain latency below 2 seconds per turn, while local models (CNN/biLSTM) will be deployed via PyTorch. The final interface will be implemented using the Streamlit framework for rapid cross-platform accessibility.
...
\midrule
CNN (Image Baseline) & & & & & \\
biLSTM (Text Baseline) & & & & & \\
\textbf{MediSim Fusion (Proposed)} & & & & & \\
\midrule
Single-Agent (Triage Baseline) & & & & & \\
\textbf{Multi-Agent (Proposed)} & & & & & \\
\bottomrule
\end{tabular}
}
...
\toprule
\textbf{Feature} & \textbf{Baseline (biLSTM/CNN)} & \textbf{Proposed (Multimodal + Multi-Agent)} \\
\midrule
...
\begin{itemize}
    ...
    \item \textbf{Phase 2}: Project Proposal and Architecture design (Current).
    \item \textbf{Phase 3}: Implementation of the Streamlit web application, data processing scripts, and training the baseline diagnostic models.
